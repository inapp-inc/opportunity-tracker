...
if [[ -z "$DEST_ENV_FILE" ]]; then
  ...
  echo "No env file found. Creating: $DEST_ENV_FILE"
  if [[ -e "$DEST_ENV_FILE" ]]; then
    # Race/edge case: it appeared between checks.
    true
  else
    umask 077
    mkdir -p "$(dirname "$DEST_ENV_FILE")" || true
    if [[ ! -w "$(dirname "$DEST_ENV_FILE")" ]]; then
      echo "Cannot write env file at $DEST_ENV_FILE" >&2
      exit 2
    fi
...
if [[ -n "$JWT_SECRET_OVERRIDE" ]]; then
  set_env_value "$DEST_ENV_FILE" "JWT_SECRET" "$JWT_SECRET_OVERRIDE"
fi

JWT_SECRET_FILE_VALUE="$(env_value "$DEST_ENV_FILE" JWT_SECRET || true)"
if [[ -z "$JWT_SECRET_FILE_VALUE" ]]; then
  echo "JWT_SECRET is missing in $DEST_ENV_FILE" >&2
  echo "Set JWT_SECRET once and keep it stable; otherwise sessions will be invalidated." >&2
  exit 3
fi
...
docker run -d --name "$CONTAINER_NAME" --restart unless-stopped --env-file "$DEST_ENV_FILE" --network host -e "NODE_ENV=production" -e "SERVE_STATIC=1" -e "APP_BASE_PATH=$BASE_PATH" -e "PUBLIC_BASE_PATH=$BASE_PATH" -e "PORT=$PORT" "$IMAGE_TAG" >/dev/null
